...
def cos_sim_set(a_set, b_set, norm=True):
    """Takes 2 sets of non-zero bits indexes from vectors a, b and returns the cosine similarity according 
    to the definition of the dot product
    """
    dot_product = len(a_set&b_set)
    if not norm: return dot_product
    norm_a = np.sqrt(len(a_set))
    norm_b = np.sqrt(len(b_set))
    return dot_product / (norm_a * norm_b)
...
for size in [125, 250, 500, 1000, 2000, 4000, 8000, 16000, 30000]:
    ...
    if algorithm == "ramezani":
        ...
        nodes_signatures = defaultdict(set)
        cur_epidemic = 0
        for l in open(ep_fn):
            z = l.strip().split('\t')
            if int(z[3]) == 0: continue
            ...
            for n in ep_nodes:
                nodes_signatures[n].add(cur_epidemic)
            cur_epidemic += 1
        ...
        for n1 in known_nodes:
            for n2 in known_nodes:
                if int(n2)>int(n1):
                    value = cos_sim_set(nodes_signatures[n1],nodes_signatures[n2])
                    if value > EPS:
