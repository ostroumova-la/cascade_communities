...
algorithm = sys.argv[1] # ClustOpt, GraphOpt
filename = algorithm

if algorithm == "ClustOpt":
...
for size in [125, 250, 500, 1000, 2000, 4000, 8000, 16000, 30000]:
    ...
    known_nodes = set()
    if algorithm == "GraphOpt":
        command = "python3 ../stub_GraphOpt.py "+ep_fn
        subprocess.check_call(command, shell=True)

        partition = dict()
        for line in open("GraphOpt_results.tsv"):
            if line.strip():
    ...
    if algorithm == "ClustOpt":
        ...
        fh = open('ClustOpt_input.tsv','w')
        for node, cluster in sorted(initial_partition.items()):
            print('%d\t%d' % (node,cluster),file=fh)
        fh.close()

        command = "python3 ../stub_ClustOpt_1step.py "+ep_fn+" ClustOpt_input.tsv"
        subprocess.check_call(command, shell=True)

        partition = dict()
        for line in open("ClustOpt_results.tsv"):
            if line.strip():
                node, comm = line.strip().split("\t")
                partition[int(node)] = int(comm)
                known_nodes.add(int(node))
